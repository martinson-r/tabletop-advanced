...
{/* {data !== undefined && (<><p>{data.game.title} hosted by {data.game.host.userName}</p> */}
        ...
        {data !== undefined && data.game.Characters.map((character) => <p><Link to={`/${character.User.id}/bio`}>{character.User.userName}</Link> as <Link to={`/characters/${character.id}`}>{character.name}</Link></p>)}
